/**
 * 관리자 전용 Lucide 아이콘 (인라인 SVG, 외부 의존 없음).
 * 앱 본체의 Material Symbols 서브셋 폰트에는 관리자에 필요한 글리프가 없어서
 * 강디 목업(docs/mockups/admin-mockup.html)의 스프라이트를 그대로 옮겼다.
 */
const PATHS: Record<string, string> = {
  ...
  notice: '<path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/>',
...
export default function AdminIcon({ name, small = false, className = "" }: { name: AdminIconName; small?: boolean; className?: string }) {
  const d = PATHS[name] || "";
  return (
    <svg
      className={`ad-ic ${small ? "ad-ic-sm" : ""} ${className}`.trim()}
      viewBox="0 0 24 24"
      aria-hidden="true"
