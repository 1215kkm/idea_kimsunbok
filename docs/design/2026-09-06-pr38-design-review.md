...
### 최종 판정: **승인** — 필수 3건 전부 반영, 권장 2건도 처리. 남은 건 전부 다음 PR 급.

| 항목 | 결과 | 근거 |
|---|---|---|
| 1 폰트 토큰 | ✓ | `admin.css:52-59` 가 `16/16/16/18/20/26/32` + 금기 사유 주석. 지적한 5곳 전부 토큰 참조라 실제로 올라감: 칩 `:228 --ad-font-sm(16)` · `.ad-sub :274 xs(16)` · `.ad-note :204 xs` · `.ad-src :293 xs` · 모달 카운터 `Modal.tsx:159 xs`. 배지 `:215 sm`, 필터칩 수 `:254 xs`, 채널 원형 `:236 xs`, 사이드바 배지 `:106 xs` 도 같이 해결 |
| 1-b 레이아웃 파손 | ✓ 없음 | 테이블: `.ad-tbl-wrap overflow-x auto` + `min-width` 는 *하한*이라 내용이 넓어지면 표가 늘고 가로 스크롤만 길어짐 — 잘리거나 겹치지 않음. KPI: 값 32 + 라벨 16 + delta 16, grid 라 4칸 높이 동일 ✓. 드로어 메타 2열(520px → 칸당 ~230px): 값이 길면 2줄로 흐를 뿐 넘침 없음, ≤640 은 `:420` 1열 ✓. 배지 `white-space: nowrap` + 카드 헤드 `flex-wrap: wrap`(`:166`) 이라 제목 옆 배지가 줄바꿈으로 흘러 안 깨짐 ✓. `.ad-btn-sm` 은 `min-height:36` 이라 16px 글자에서 ~37px 로 스스로 커짐(고정 height 아님) ✓ |
| 2 `.ad-term.warn` | ✓ | `:299-300` — `.beta`(`:296-297`) 와 동일 구조에 warning 쌍(`--ad-warning` 보더 / `--ad-warning-light` 배경 / `--ad-chip-orange-text` 값). 총량 검산에서 빨강(베타) · 주황(경고) · 회색(제로) · 네이비(좌변) 4단으로 구분됨 |
| 3 `.ad-input:disabled` | ✓ | `:357` — 배경·글자·커서에 `border-color` 까지. 드로어 일일 상한이 종료 캠페인에서 잠긴 게 눈에 보임 |
| 4 햄버거 → PageHeader | ✓ **수용** | `PageHeader.tsx:17,22` 인라인 + `.ad-topbar .ad-title` flex 래퍼 신설(`:118`, `min-width:0` 로 축소 허용), `AdminContext:16 openSidebar`, `layout.tsx:189-199` 별도 행 제거. 9개 화면 전부 PageHeader 사용 확인(9파일). 내 "조건부 폴백" 안보다 이게 낫다 — 폴백은 조건을 잘못 쓰면 두 개가 보이고, 지금은 "관리자 페이지는 PageHeader 를 쓴다" 가 규칙이 됨. `PageHeader.tsx:21` 주석이 그 규칙을 남기고 있어 충분 |
| 5-a 탭 카운트 칩 | ✓ | `.ad-tabs .ad-n :346 min-width:28px` 고정폭 + `reward:384` 로딩 중엔 `…`, 잘림은 `10+` — 처음부터 폭이 안 변해 옆 탭이 안 밀림 (원칙 3 해소) |
| 5-b ledger 진입 1개 | ✓ | 대시보드에 남은 건 `page.tsx:243 "총량 검산에서 원인 보기"` 하나. 총량 검산 카드의 "상세 보기" 제거 확인 (원칙 4 해소) |
| 덤 | ✓ | `ledger:143` 의미 없는 삼항 `fmtP(t.op === "−" ? t.value : t.value)` → `fmtP(t.value)` 정리됨 |

### 다음 PR 로 넘김 (이번 머지 차단 아님)

- 다크 테마 누수 — `--ad-hover/border/gray-bg/primary-rgb` 라이트 고정. `/admin` 라이트 강제 또는 `[data-theme="dark"]` 재선언.
- `--ad-border(#E8EAF0)` → `var(--divider)` 로 앱과 한 소스.
- Suspense·권한 확인 폴백 텍스트 → KPI 스켈레톤으로 통일.
- 캠페인 테이블 `min-width:1180` 은 16px 기준으로 다시 재면 1280~1320 이 실측에 가까움. 지금도 스크롤로 동작하니 P1 에 실제 화면 보고 조정.
- 드로어 메타의 키(`.k`)와 값이 이제 같은 16px — 색·굵기로만 구분됨. 읽기 문제는 없으나 키를 `--text-muted` + 500 로 한 단계 더 눌러 두면 스캔이 빨라짐.
